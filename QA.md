...
## 3. Share extension flow
- [ ] In Photos.app, sharing a Live Photo shows "Giffer" in the share sheet
- [ ] Tapping Giffer in share sheet shows spinner, then opens main app
- [ ] Main app opens directly to editor with the shared photo loaded
- [ ] Sharing a still photo: Giffer either hidden or shows graceful "no Live Photo" error
- [ ] Sharing a regular video: Giffer not offered (image-only activation rule)
- [ ] Sharing while Giffer is already open: replaces current photo
- [ ] After share completes, no leftover files in shared container after 1 hour (verify via Files app or device debug)

## 4. Playback modes (top bar)
...
- [ ] Share extension with no Live Photo component: shows error and dismisses
...
## 16. App Store Connect prerequisites (off-device)
